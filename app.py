...
with st.sidebar.form(key="my_form"):
    ...
    min_sp_def, max_sp_def = st.select_slider(
        "Special Defense", range(251), value=[0, 250]
    )
    min_sp_atk, max_sp_atk = st.select_slider(
        "Special Attack", range(251), value=[0, 250]
    )
...
def display_radar_chart(match):
    ...
    fig = px.line_polar(
        df_stats, r="stats", theta=df_stats.index, line_close=True, range_r=[0, 250]
    )
    st.plotly_chart(fig)
...
def display_similar_pokemons(match):
    ...
    df_stats_all = df_stats_all.rename(
        columns={
            "hp": "HP",
            "attack": "Attack",
            "defense": "Defense",
    ...
    diff_df = pd.DataFrame(
        df_stats_all.values - df_stats.values, index=df_stats_all.index
    )
    ...
    similar_pokemons = norm_df.nsmallest(21)[
        1:22
    ].index  # index [1:22] so it does not show itself
    ...
    for row in similar_pokemons_df.iterrows():
        ...
        # display radar chart
        fig = px.line_polar(
            row[1], r=name, theta=row[1].index, line_close=True, range_r=[0, 255]
        )
        st.plotly_chart(fig)
...
# we do not display every bit of information about the search Pokemons as the list may be huge and the app can hang
    ...
    searched_pokemons_df = df_stats_all[
        ...
        & (df_stats_all["Special Attack"] >= min_sp_atk)
        & (df_stats_all["Special Attack"] <= max_sp_atk)
        & (df_stats_all["Special Defense"] >= min_sp_def)
        & (df_stats_all["Special Defense"] <= max_sp_def)
